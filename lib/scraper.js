...
const downloadMp4 = async (Link) => {
  try {
    let p = await ytdl.downloadMedia(Link, 'video', 5);
    return {
      status: true,
      type: p.type,
    ...
    console.error(err);
    return { status: false, creator: "Hello Line", error: 'Gagal mengunduh video MP4.' };
  }
};

const downloadMp3 = async (Link) => {
  try {
    let p = await ytdl.downloadMedia(Link, 'audio', 4);
    return {
      status: true,
      type: p.type,
      duration: p.durationLabel,
    ...
    console.error(err);
    return { status: false, creator: "Hello Line", error: 'Gagal mengunduh audio MP3.' };
  }
};

module.exports = { terabox, ytdl, downloadMp3, downloadMp4 }
